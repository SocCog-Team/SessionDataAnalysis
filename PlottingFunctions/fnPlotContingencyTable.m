function [ fh_cur_contingency_table, cur_group_names ] = fnPlotContingencyTable( table, row_names, column_names, group_by_string, title_string, plot_type, Psymbol_by_group, cols_idx_per_symbol, img_fqn_by_group_list )
...
if (size(group_names_and_n, 1) == 1) && (show_xlabels)
	xlabel('Group Numbers', 'FontSize', 18, 'FontWeight', 'bold');
end
...
if ~isempty(Psymbol_by_group)
	...
	switch size(group_names_and_n, 2)
		...
		case {21 23 24 25 26 27 28 29 30}
			symbol_font_size = 12;
			y_row_offset_factor = 0.05;	% fraction of plot space to give to each y symbol row
