function [ out_figure_handle, merged_classifier_char_string ] = fn_plot_SOC_histogram_by_switches( in_figure_handle, RT_by_switch_struct_list, selected_switches_list, RT_by_switch_title_prefix_list, RT_by_switch_switch_pre_bins_list, RT_by_switch_switch_n_bins_list, RT_by_switch_color_list, aggregate_type_list, x_label_string, y_label_string)
...
if ~exist('x_label_string', 'var') || isempty(x_label_string)
	x_label_string = 'Choice switches';
end


if ~exist('y_label_string', 'var') || isempty(y_label_string)
	y_label_string = 'Share of choices';
end
...
for i_switch_struct = 1 : n_data_structs
	...
	set(gca(), 'XTickLabel', current_xtick_label, 'FontWeight', 'bold', 'FontSize', font_size);
	
	if (plot_separately)
		fn_label_and_scale_plot(min_y_lim, max_y_lim, max_RT_sample, min_RT_sample, font_weight, font_size, x_label_string, y_label_string);
	end
...
function [ ] = fn_label_and_scale_plot( min_y_lim, max_y_lim, max_RT_sample, min_RT_sample, font_weight, font_size, x_label_string, y_label_string )
% scale the axis.
y_lim = get(gca(), 'YLim');
...
set(gca(), 'YLim', y_lim);
set(gca(), 'YTick', [0.0 0.5 1.0]);


xlabel(x_label_string, 'Interpreter', 'none', 'FontWeight', font_weight, 'FontSize', font_size);
ylabel(y_label_string, 'Interpreter', 'none', 'FontWeight', font_weight, 'FontSize', font_size);
